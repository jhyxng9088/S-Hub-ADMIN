...
function auditLabel(action) {
  ...
  if (action === 'admin_bootstrap') return '관리자 등록'
  if (action === 'admin_super_admin_verified') return '최고 관리자 인증'
  return action || '관리 작업'
}
...
function AccessGate({ uid, onActivated }) {
  ...
  async function activate(event) {
    ...
    try { await bootstrapAdmin(secret); setSecret(''); await onActivated() }
    catch (error) { setMessage(error.message || '관리자 등록에 실패했어.') }
    finally { setBusy(false) }
  }
  return <main className="gate-page"><section className="gate-card"><div className="admin-mark">S</div><p className="eyebrow">S-Hub Admin</p><h1>관리자 권한이 필요해</h1><p className="muted">관리자 비밀번호를 맞게 입력하면 이 기기는 최고 관리자 권한으로 등록돼.</p><code className="uid-box">{uid}</code><form onSubmit={activate} className="bootstrap-form"><input type="password" value={secret} onChange={(event) => setSecret(event.target.value)} placeholder="관리자 비밀번호" autoComplete="off"/><button disabled={busy || !secret.trim()}>{busy ? '확인 중…' : '최고 관리자로 로그인'}</button></form>{message ? <p className="error-copy">{message}</p> : null}</section></main>
}
...
function compactObject(value) {
  ...
  if (Array.isArray(value)) return value.map((item) => compactObject(item)).join(' · ')
  return Object.entries(value).map(([key, item]) => `${key}: ${compactObject(item)}`).join(' / ')
}

function ContentItems({ items, empty }) {
  if (!items?.length) return <div className="empty-state compact">{empty}</div>
  return <div className="class-content-list">{items.map((item) => {
    const title = item.title || item.name || item.subject || item.content || item.text || item.id
    const subtitle = item.dueLabel || item.dueDate || item.date || item.startDate || item.time || item.description || item.memo || ''
    const rest = Object.fromEntries(Object.entries(item).filter(([key]) => !['id','title','name','subject','content','text','dueLabel','dueDate','date','startDate','time','description','memo'].includes(key)))
    return <details className="class-content-item" key={item.id}><summary><div><strong>{String(title || '항목')}</strong>{subtitle ? <span>{compactObject(subtitle)}</span> : null}</div><small>상세 보기</small></summary><div className="class-content-raw">{Object.keys(rest).length ? Object.entries(rest).map(([key,value]) => <div key={key}><span>{key}</span><b>{compactObject(value)}</b></div>) : <p>추가 정보가 없어.</p>}</div></details>
  })}</div>
}

function TimetableView({ timetable }) {
  if (!timetable) return <div className="empty-state compact">설정된 시간표가 없어.</div>
  const weekly = timetable.weeklySchedule && typeof timetable.weeklySchedule === 'object' ? timetable.weeklySchedule : null
  return <div className="timetable-admin-view">{weekly ? Object.entries(weekly).map(([day, periods]) => <section key={day}><h4>{day}</h4><div>{Array.isArray(periods) ? periods.map((period, index) => <span key={`${day}-${index}`}><b>{index + 1}교시</b>{compactObject(period)}</span>) : <span>{compactObject(periods)}</span>}</div></section>) : <div className="class-content-raw">{Object.entries(timetable).filter(([key]) => key !== 'id').map(([key,value]) => <div key={key}><span>{key}</span><b>{compactObject(value)}</b></div>)}</div>}</div>
}

function ClassSheet({ row, onClose, onStudents }) {
  const [details, setDetails] = useState(null)
  const [error, setError] = useState('')
  useEffect(() => { let active = true; setDetails(null); setError(''); loadClassDetails(row.classId).then((value) => active && setDetails(value)).catch((e) => active && setError(e.message)); return () => { active = false } }, [row.classId])
  return <div className="sheet-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="detail-sheet class-sheet"><div className="sheet-handle"/><div className="detail-title"><div><p className="eyebrow">반 상세</p><h2>{row.grade ? `${row.grade}학년 ` : ''}{row.classNumber ? `${row.classNumber}반` : row.classId}</h2><p>리마인더 · 시간표 · 학사일정</p></div><button className="close-button" onClick={onClose}>×</button></div><div className="status-strip"><span>학생 {row.students}명</span><span>리마인더 {row.reminders}개</span><span>학사일정 {row.academics}개</span></div>{error ? <p className="error-copy">{error}</p> : null}{!details ? <div className="loading-card">반 데이터를 불러오는 중…</div> : <><section className="sheet-section class-content-section"><div className="sheet-section-title"><h3>리마인더</h3><span>{details.reminders?.length || 0}개</span></div><ContentItems items={details.reminders} empty="등록된 리마인더가 없어."/></section><section className="sheet-section class-content-section"><div className="sheet-section-title"><h3>시간표</h3><span>{details.timetable ? '설정됨' : '미설정'}</span></div><TimetableView timetable={details.timetable}/></section><section className="sheet-section class-content-section"><div className="sheet-section-title"><h3>학사일정</h3><span>{details.academicEvents?.length || 0}개</span></div><ContentItems items={details.academicEvents} empty="등록된 학사일정이 없어."/></section></>}<div className="sheet-actions"><button className="secondary-action" onClick={() => { onClose(); onStudents(row) }}>이 반 학생 보기</button></div></section></div>
}

function ClassCards({ classes, onOpenClass }) {
  const [selectedClass, setSelectedClass] = useState(null)
  return <><div className="class-card-grid">{classes.length ? classes.map((row) => <button className="class-detail-card" key={row.classId} onClick={() => setSelectedClass(row)}><div className="class-card-head"><div><span className="eyebrow">{row.grade ? `${row.grade}학년` : '학년 미수집'}</span><h3>{row.classNumber ? `${row.classNumber}반` : row.classId}</h3></div><strong>{row.students}명</strong></div><div className="class-health-line"><span>7일 활성률</span><b>{percent(row.active7d, row.students)}</b><div><i style={{ width: percent(row.active7d, row.students) }}/></div></div><dl className="class-stats"><div><dt>온라인</dt><dd>{row.online}</dd></div><div><dt>24시간</dt><dd>{row.activeToday}</dd></div><div><dt>30일 미접속</dt><dd>{row.inactive30d}</dd></div><div><dt>최근 7일 수정</dt><dd>{row.activity7d}</dd></div><div><dt>리마인더</dt><dd>{row.reminders}</dd></div><div><dt>학사일정</dt><dd>{row.academics}</dd></div></dl><div className="class-footer"><span>{row.timetableConfigured ? `시간표 설정 · ${relativeTime(row.timetableUpdatedAt)}` : '시간표 미설정'}</span><span>눌러서 전체 내용 보기</span></div></button>) : <div className="empty-state">반 데이터가 없어.</div>}</div>{selectedClass ? <ClassSheet row={selectedClass} onClose={() => setSelectedClass(null)} onStudents={onOpenClass}/> : null}</>
}
...
function StudentSheet({ user, admin, onClose, onChanged }) {
  ...
  return <div className="sheet-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="detail-sheet expanded"><div className="sheet-handle"/><div className="detail-title"><div><p className="eyebrow">학생 상세</p><h2>{user.name}</h2><p>{user.grade ? `${user.grade}학년 · ` : ''}{user.classNumber || '?'}반 · {user.studentNumber || '?'}번</p></div><button className="close-button" onClick={onClose}>×</button></div><div className="status-strip"><span className={user.online ? 'status-live' : ''}>{user.online ? '현재 온라인' : `마지막 접속 ${relativeTime(user.effectiveLastSeenAt)}`}</span><span>{user.status === 'active' ? '계정 정상' : '계정 비활성'}</span></div><div className="detail-grid"><div><span>최초 가입</span><strong>{formatDateTime(user.createdAt)}</strong></div><div><span>연결 인증 계정</span><strong>{details ? `${details.identityCount}개` : '불러오는 중'}</strong></div><div><span>최근 7일 수정</span><strong>{user.activity7d || 0}회</strong></div><div><span>전체 수정 기록</span><strong>{details ? `${details.activityTotal}회` : '불러오는 중'}</strong></div><div><span>푸시 등록</span><strong>{details ? `${details.pushDevices}대` : `${user.pushDevices || 0}대`}</strong></div><div><span>리마인더 상태</span><strong>{todo ? `${todo.completed}/${todo.total} 완료` : '불러오는 중'}</strong></div><div><span>앱 버전</span><strong>{user.appVersion || '미수집'}</strong></div><div><span>실행 형태</span><strong>{user.displayMode || '미수집'}</strong></div></div><section className="sheet-section"><div className="sheet-section-title"><h3>최근 수정 활동</h3><span>최대 30개</span></div><ActivityList items={details?.recentActivity}/></section><section className="sheet-section"><div className="sheet-section-title"><h3>연결된 인증 계정</h3><span>{details?.identityCount || user.identityCount || 0}개</span></div><div className="identity-list">{details?.identities?.length ? details.identities.map((item) => <div className="identity-item" key={item.uid}><div><strong>{shortUid(item.uid)}</strong><span>{item.disabled === true ? '비활성' : item.disabled === false ? '정상' : 'Auth 기록 없음'}</span></div><div><small>가입 {formatDateTime(item.createdAt)}</small><small>최근 로그인 {relativeTime(item.lastSignInAt)}</small></div></div>) : <div className="empty-state compact">계정 정보를 불러오는 중이야.</div>}</div></section><details className="technical-details"><summary>식별 정보</summary><dl><dt>대표 UID</dt><dd>{user.uid}</dd><dt>Student Key</dt><dd>{user.studentKey || '없음'}</dd><dt>마지막 데이터 갱신</dt><dd>{formatDateTime(user.updatedAt)}</dd></dl></details>{error ? <p className="error-copy">{error}</p> : null}<div className="sheet-actions"><button className="secondary-action" disabled={busy} onClick={toggleStatus}>{user.status === 'active' ? '학생 비활성화' : '학생 복구'}</button>{admin.role === 'super_admin' ? <button className={`danger-action ${confirmDelete ? 'confirming' : ''}`} disabled={busy} onClick={removePermanently}>{confirmDelete ? '정말 영구 삭제' : '영구 삭제'}</button> : null}</div>{confirmDelete ? <p className="danger-note">영구 삭제는 연결된 Firebase 인증 계정과 학생 개인 데이터를 제거해. 반 공용 리마인더·시간표·학사일정은 삭제하지 않아.</p> : null}</section></div>
}
